...
# ss(lambda) = nLw(lambda) - nLw(lambda_-) - (nLw(lambda_+) - nLw(lambda_-)) * (lambda - lambda_-)/(lambda_+ - lambda_-)
# lambda_- is next lower bandwidth from lambda
# lambda_+ is next higher bandwidth from lambda
# negative ss(490) = Kbrevis bloom

### benthic resuspension of chl
# chl(res) ~ rsr(670) anomaly calculated similar to chl anomaly
# then multiply  rsr(670)_anom by 200 ug sr l-1 to convert ro chlorophyll (Wynne et al. 2005, 2006)
# corrected_chlorophyll_anomaly = original_chl_anomaly - chl(res)
# standard chl_oc4 overestimates chlorophyll in coastal FL; use Stumpf et al. 2000
...
# threshold of above 0.12 W m−2 μm−1 sr−1 were associated with K. brevis concentration between 104 and 106 cells L−1 (Hu et al. 2005)
# FLH anomaly maybe suitable replacement for chl_anom (Tomlinson et al. 2009)
